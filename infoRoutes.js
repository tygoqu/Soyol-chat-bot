...
function registerInfoRoutes(app, adminSecret) {
  app.get('/info', async (req, res) => {
    try {
      ...
      const serviceHtml = (info.services || '')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean)
      ...
      const announcementHtml = info.announcement
        ? `<div class="card"><h2>Зар мэдээ</h2><p>${esc(info.announcement)}</p></div>`
        : '';
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hero p, .card p { line-height: 1.7; color: #4b5563; }
...
<head>
  ...
  <style>
    body { font-family: Arial, sans-serif; background: #f7f3f8; padding: 24px; }
    .card { max-width: 520px; margin: 60px auto; background: #fff; border: 1px solid #eadff0; border-radius: 18px; padding: 24px; }
  </style>
</head>
<body>
  <div class="card">
