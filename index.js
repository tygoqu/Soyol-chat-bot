...
async function initializeSheets() {
  ...
  await ensureHeadersIfEmpty(SUBSCRIBERS_SHEET, ['subscriber_id', 'date_added']);
  await ensureHeadersIfEmpty(BOOKINGS_SHEET, ['booking_id', 'created_at', 'customer_name', 'phone', 'category_name', 'service_name', 'date', 'time', 'note', 'status', 'service_duration', 'service_price']);
}

async function loadSubscribers() {
  try {
    const sheets = getSheets();
    const res = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${SUBSCRIBERS_SHEET}!A:A` });
    const rows = res.data.values || [];
    return new Set(rows.map(r => (r[0] || '').trim()).filter(v => v && v !== 'subscriber_id'));
  } catch (e) {
...
async function addSubscriber(id) {
  try {
    ...
    await sheets.spreadsheets.values.append({
      ...
      valueInputOption: 'RAW',
      requestBody: { values: [[id, new Date().toISOString()]] },
    });
  } catch (e) {
...
async function getAllBookings() {
  const sheets = getSheets();
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${BOOKINGS_SHEET}!A:L` });
  const rows = res.data.values || [];
  return rows
    ...
    if (body.object === 'page') {
      for (const entry of body.entry || []) {
        ...
        for (const event of entry.messaging || []) {
          ...
          else if (event.message?.text) await sendMainMenu(id);
        }
      }
...
app.get('/booking-unavailable', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to load unavailable times' });
  }
});
...
app.post('/booking-submit', async (req, res) => {
  try {
    ...
    if (!payload.categoryName || !payload.serviceName || !payload.date || !payload.time || !payload.customerName || !payload.phone) {
    ...
    if (unavailable.includes(payload.time)) {
      return res.status(409).json({ error: 'Энэ цаг аль хэдийн захиалагдсан байна. Өөр цаг сонгоно уу.', code: 'SLOT_TAKEN' });
    }
...
<title>Soyol Spa Salon — Broadcast</title>
<style>
  * { box-sizing: border-box; }
  body { margin: 0; font-family: Arial, sans-serif; background: #f7f3f8; color: #241b2f; min-height: 100vh; display: grid; place-items: center; padding: 24px; }
  .card { width: 100%; max-width: 560px; background: #fff; border: 1px solid #eadff0; border-radius: 18px; box-shadow: 0 18px 48px rgba(79, 32, 104, 0.10); padding: 28px; }
  .brand { text-align: center; margin-bottom: 22px; }
  .brand img { width: 84px; height: 84px; object-fit: contain; display: block; margin: 0 auto 12px; }
  .brand h1 { margin: 0; font-size: 24px; }
  .brand p { margin: 6px 0 0; color: #7d6c8e; font-size: 14px; }
  .stats { display: flex; justify-content: space-between; align-items: center; padding: 14px 16px; background: #6f3f89; color: #fff; border-radius: 14px; margin-bottom: 18px; }
  .stats strong { font-size: 26px; }
  label { display: block; margin: 12px 0 8px; font-size: 13px; font-weight: 700; }
  ...
  button { border: 0; padding: 13px 16px; margin-top: 14px; background: #7b2d8b; color: #fff; font-weight: 700; cursor: pointer; }
  button:disabled { opacity: 0.6; cursor: not-allowed; }
  .hidden { display: none; }
  .note, .toast { font-size: 13px; color: #7d6c8e; margin-top: 10px; }
</style>
</head>
<body>
  <div class="card">
    <div class="brand">
      <img src="/logo.png" alt="Soyol Spa Salon logo">
      <h1>Soyol Spa Salon</h1>
      <p>Broadcast admin</p>
    </div>
    <div id="loginSection">
  ...
  <script>
    ...
    async function doLogin() {
      ...
      toast.textContent = '';
      if (!secret) { toast.textContent = 'Password оруулна уу.'; return; }
      const r = await fetch('/stats?secret=' + encodeURIComponent(secret));
      const data = await r.json();
      if (!r.ok) { toast.textContent = data.error || 'Нэвтрэх үед алдаа гарлаа.'; return; }
    ...
    async function sendBroadcast() {
      ...
      toast.textContent = '';
      if (!message) { toast.textContent = 'Мессеж хоосон байна.'; return; }
      btn.disabled = true;
      try {
        const r = await fetch('/broadcast', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ secret, message }) });
        const data = await r.json();
        if (!r.ok) toast.textContent = data.error || 'Илгээх үед алдаа гарлаа.';
        else { toast.textContent = String(data.sent) + ' subscriber руу амжилттай илгээлээ.'; document.getElementById('msgInput').value = ''; }
      } catch (e) {
        ...
        attachment: {
          ...
          payload: {
            ...
            buttons: [
              ...
              { type: 'web_url', title: 'Цаг захиалах', url: 'https://soyol-chat-bot.onrender.com/booking', webview_height_ratio: 'full' },
      ...
      message: {
        attachment: {
          ...
          payload: {
            ...
            elements: [
              { title: 'Гоо сайхны үйлчилгээ', image_url: 'https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?q=80&w=2670&auto=format&fit=crop', subtitle: 'Арьс арчилгаа, нүүр арчилгаа, массаж', buttons: [{ type: 'postback', title: 'Дэлгэрэнгүй', payload: 'BEAUTY_SERVICE' }] },
              { title: 'Үсчин', image_url: 'https://images.unsplash.com/photo-1562322140-8baeececf3df?q=80&w=1200&auto=format&fit=crop', subtitle: 'Үс тайралт, будаг, янзалгаа', buttons: [{ type: 'postback', title: 'Дэлгэрэнгүй', payload: 'HAIR_SERVICE' }] },
              { title: 'Маникюр, педикюр', image_url: 'https://images.unsplash.com/photo-1604654894610-df63bc536371?q=80&w=1200&auto=format&fit=crop', subtitle: 'Гар, хөлийн арчилгаа', buttons: [{ type: 'postback', title: 'Дэлгэрэнгүй', payload: 'NAIL_SERVICE' }] },
              { title: 'Сормуус, хөмсөг', image_url: 'https://images.unsplash.com/photo-1512496015851-a90fb38ba796?q=80&w=1200&auto=format&fit=crop', subtitle: 'Сормуус, хөмсөгний үйлчилгээ', buttons: [{ type: 'postback', title: 'Дэлгэрэнгүй', payload: 'EYELASH_SERVICE' }] },
              { title: 'Чих цоолох, персинг', image_url: 'https://images.unsplash.com/photo-1596944948860-67d8f0d2f30e?q=80&w=1200&auto=format&fit=crop', subtitle: 'Персинг үйлчилгээ', buttons: [{ type: 'postback', title: 'Дэлгэрэнгүй', payload: 'PIERCING_SERVICE' }] },
              { title: 'Мэнгэ, үү, ургацаг авах', image_url: 'https://images.unsplash.com/photo-1515377905703-c4788e51af15?q=80&w=1200&auto=format&fit=crop', subtitle: 'Арьсны жижиг ургацаг авах үйлчилгээ', buttons: [{ type: 'postback', title: 'Дэлгэрэнгүй', payload: 'REMOVAL_SERVICE' }] },
              { title: 'Үсний эмчилгээ', image_url: 'https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?q=80&w=1200&auto=format&fit=crop', subtitle: 'Үсний тэжээл, сэргээх үйлчилгээ', buttons: [{ type: 'postback', title: 'Дэлгэрэнгүй', payload: 'HAIRTREATMENT_SERVICE' }] },
              { title: 'Үсний бүтээгдэхүүн', image_url: 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?q=80&w=1200&auto=format&fit=crop', subtitle: 'Шампунь, ангижруулагч, маск', buttons: [{ type: 'postback', title: 'Дэлгэрэнгүй', payload: 'HAIR_PRODUCT' }] }
            ]
          }
        }
      }
      ...
      message: {
        attachment: {
          ...
          payload: {
            template_type: 'generic',
            elements: [
              { title: 'Энгийн массаж', image_url: 'https://images.unsplash.com/photo-1731514771613-991a02407132?q=80&w=1287&auto=format&fit=crop', subtitle: 'Энгийн массаж\nҮнэ: 65,000₮', buttons: [{ type: 'web_url', title: 'Цаг авах', url: 'https://soyol-chat-bot.onrender.com/booking', webview_height_ratio: 'full' }] },
              { title: 'Гуаша массаж', image_url: 'https://assets.clevelandclinic.org/transform/LargeFeatureImage/b9bd499d-f631-42c3-87c6-4ba1bd3ef9f3/guasha-2177381155', subtitle: 'Гуаша массаж\nҮнэ: 85,000₮', buttons: [{ type: 'web_url', title: 'Цаг авах', url: 'https://soyol-chat-bot.onrender.com/booking', webview_height_ratio: 'full' }] },
              { title: 'Miracle CO2', image_url: 'https://www.lerden.ru/assets/images/data/photo-2024-09-05-17-50-40.webp', subtitle: 'Miracle CO2\nҮнэ: 65,000₮ - 85,000₮', buttons: [{ type: 'web_url', title: 'Цаг авах', url: 'https://soyol-chat-bot.onrender.com/booking', webview_height_ratio: 'full' }] },
              { title: 'Carbon peel', image_url: 'https://elaraclinics.com/wp-content/uploads/2023/12/close-up-of-cosmetologist-s-hand-making-hardware-c-2023-11-27-05-28-55-utc-1024x683.jpg', subtitle: 'Carbon peel\nҮнэ: 85,000₮', buttons: [{ type: 'web_url', title: 'Цаг авах', url: 'https://soyol-chat-bot.onrender.com/booking', webview_height_ratio: 'full' }] },
              ...
              { title: 'Батга цэвэрлэгээ', image_url: 'https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?q=80&w=1200&auto=format&fit=crop', subtitle: 'Батга цэвэрлэгээ\nҮнэ: 85,000₮ - 120,000₮', buttons: [{ type: 'web_url', title: 'Цаг авах', url: 'https://soyol-chat-bot.onrender.com/booking', webview_height_ratio: 'full' }] },
              { title: 'Carboxy', image_url: 'https://images.unsplash.com/photo-1515377905703-c4788e51af15?q=80&w=1200&auto=format&fit=crop', subtitle: 'Carboxy\nҮнэ: 65,000₮', buttons: [{ type: 'web_url', title: 'Цаг авах', url: 'https://soyol-chat-bot.onrender.com/booking', webview_height_ratio: 'full' }] }
            ]
          }
        }
      }
...
async function sendHairCarousel(id) { return reply(id, 'Үсчний үйлчилгээ удахгүй нэмэгдэнэ.'); }
async function sendEyebrowCarousel(id) { return reply(id, 'Хөмсөгний үйлчилгээ удахгүй нэмэгдэнэ.'); }
async function sendEyelashCarousel(id) { return reply(id, 'Сормуусны үйлчилгээ удахгүй нэмэгдэнэ.'); }
async function sendNailCarousel(id) { return reply(id, 'Маникюр, педикюр удахгүй нэмэгдэнэ.'); }
async function sendHairProductCarousel(id) { return reply(id, 'Үсний бүтээгдэхүүн удахгүй нэмэгдэнэ.'); }
async function sendHairTreatmentCarousel(id) { return reply(id, 'Үсний эмчилгээ удахгүй нэмэгдэнэ.'); }
async function sendPiercingCarousel(id) { return reply(id, 'Персинг үйлчилгээ удахгүй нэмэгдэнэ.'); }
async function sendRemovalCarousel(id) { return reply(id, 'Мэнгэ, үү, ургацаг авах үйлчилгээ удахгүй нэмэгдэнэ.'); }
