      ...
      for (const event of entry.messaging || []) {
        ...
        try {
          if (event.postback?.payload === 'GET_STARTED') {
            await getName(id);
          } else if (event.postback?.payload === 'MAIN_MENU') {
            await getName(id);
          } else if (event.postback?.payload === 'SCHEDULE') {
            await scheduleMenu(id);
              ...
              buttons: [
                {
                  type: 'web_url',
                  title: 'Google Maps',
                  url: 'https://maps.app.goo.gl/r4ruwUMZnxv79oVa7'
                }
              ]
            }
          }
...
app.listen(PORT, async () => {
  ...
  // This sets the Messenger bottom-right menu automatically on startup.
  // You can leave it on.
  try {
    await setPersistentMenu();
  } catch (error) {
